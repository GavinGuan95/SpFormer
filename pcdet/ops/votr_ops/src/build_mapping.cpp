...
int downsample_with_hash_with_features_wrapper(int x_max, int y_max, int z_max, int x_stride, int y_stride, int z_stride,
                                    int num_voxels, int num_ds_voxels, int hash_size,
                                    at::Tensor v_indices_tensor, at::Tensor ds_v_indices_features_tensor, at::Tensor xyz_to_vidx_tensor, at::Tensor vcount_tensor) {
    CHECK_INPUT(v_indices_tensor);
    CHECK_INPUT(ds_v_indices_features_tensor);
    CHECK_INPUT(xyz_to_vidx_tensor);
    CHECK_INPUT(vcount_tensor);

    const int *v_indices = v_indices_tensor.data<int>();
    int *ds_v_indices_features = ds_v_indices_features_tensor.data<int>();
    int *xyz_to_vidx = xyz_to_vidx_tensor.data<int>();
    int *vcount = vcount_tensor.data<int>();

    downsample_with_hash_with_features_kernel_launcher(x_max, y_max, z_max, x_stride, y_stride, z_stride, num_voxels, num_ds_voxels, hash_size,
                                                v_indices, ds_v_indices_features, xyz_to_vidx, vcount);
    return 1;
}

int downsample_with_hash_with_features_reuse_densemap_wrapper(int x_max, int y_max, int z_max, int x_stride, int y_stride, int z_stride,
    ...
    downsample_with_hash_with_features_reuse_densemap_kernel_launcher(x_max, y_max, z_max, x_stride, y_stride, z_stride,
                                                accu_x_stride, accu_y_stride, accu_z_stride, num_voxels, num_ds_voxels, hash_size,
                                                v_indices, ds_v_indices_features, xyz_to_vidx, vcount);
    return 1;
}
...
int downsample_with_hash_with_features_reuse_densemap_stridetag_wrapper(int x_max, int y_max, int z_max, int x_stride, int y_stride, int z_stride,
    ...
    downsample_with_hash_with_features_reuse_densemap_stridetag_kernel_launcher(x_max, y_max, z_max, x_stride, y_stride, z_stride,
                                                accu_x_stride, accu_y_stride, accu_z_stride, num_voxels, num_ds_voxels, hash_size,
                                                v_indices, ds_v_indices_features, xyz_to_vidx, vcount);
    return 1;
}