...
__device__ int hash_table_find_reuse_densemap(int &key, int accu_stride, int &hash_size, const int *xyz_to_vidx) {
    int hash_idx = simple_hash(key, hash_size);
    int v_idx = EMPTY_KEY;
    int prob_cnt = 0;
    while (true) {
        // found
        if (xyz_to_vidx[hash_idx * (2+1) + 2] == accu_stride && xyz_to_vidx[hash_idx * (2+1) + 0] == key) {
            v_idx = xyz_to_vidx[hash_idx * (2+1) + 1];
            break;
        }
        // empty or previous key, not found
        if (xyz_to_vidx[hash_idx * (2+1) + 2] != accu_stride){
            break;
        }
        // linear probing
        hash_idx = (hash_idx + 1) % hash_size;
...
__device__ int hash_table_find_reuse_densemap_stridetag(int &key, int accu_stride, int &hash_size, const int *xyz_to_vidx) {
    int hash_idx = simple_hash(key, hash_size);
    int v_idx = EMPTY_KEY;
    int prob_cnt = 0;
    while (true) {
        // found
        int prev_stride = (xyz_to_vidx[hash_idx*2 + 0] & 0x3c000000) >> 26;
        int cur_stride = log2(accu_stride);
        int key_insert = key | (cur_stride << 26);
        if (xyz_to_vidx[hash_idx * 2 + 0] == key_insert) {
            v_idx = xyz_to_vidx[hash_idx * 2 + 1];
            break;
        }
        // empty or previous key, not found
        if (prev_stride != cur_stride){
            break;
        }
        // linear probing
        hash_idx = (hash_idx + 1) % hash_size;
...
__global__ void subm_strided_attention_with_hash_reuse_densemap_kernel(int x_max, int y_max, int z_max, int accu_x_stride, int accu_y_stride, int accu_z_stride, int num_voxels, int attend_size, int num_range, int hash_size,
                                                            int *attend_indices, const int *v_indices, const int *xyz_to_vidx, const int *range_spec) {
    /*
        attend_indices: [num_voxels, attend_size] for gather attend indices
        v_indices: [num_voxels, 4] bs + zyx indices of voxels
    ...
    */
    ...
    int y_idx = v_indices[th_idx * 4 + 2];
    int x_idx = v_indices[th_idx * 4 + 3];
//    int or_z_idx = z_idx * accu_z_stride;
//    int or_y_idx = y_idx * accu_y_stride;
//    int or_x_idx = x_idx * accu_x_stride;
//    int or_z_max = z_max * accu_z_stride;
//    int or_y_max = y_max * accu_y_stride;
//    int or_x_max = x_max * accu_x_stride;
    //if (or_x_idx >= x_max || or_x_idx < 0 || or_y_idx < 0 || or_y_idx >= y_max || or_z_idx < 0 || or_z_idx >= z_max) return;
    if (x_idx >= x_max || x_idx < 0 || y_idx < 0 || y_idx >= y_max || z_idx < 0 || z_idx >= z_max) return;
    xyz_to_vidx += bs_idx * hash_size * (2+1);
            ...
            for (int sz_idx = z_idx - z_offset; sz_idx <= z_idx + z_offset; sz_idx += z_step){
                ...
                for (int sy_idx = y_idx - y_offset; sy_idx <= y_idx + y_offset; sy_idx += y_step){
                    ...
                    for (int sx_idx = x_idx - x_offset; sx_idx <= x_idx + x_offset; sx_idx += x_step){
                        if (sx_idx >= x_max || sx_idx < 0) continue;
                        int skey = sx_idx * y_max * z_max + sy_idx * z_max + sz_idx;
                        int sv_idx = hash_table_find_reuse_densemap(skey, accu_x_stride, hash_size, xyz_to_vidx);
                        if (sv_idx != EMPTY_KEY) { // found non-empty index
                            if (num_samples >= attend_size) return; // full and return
                            attend_indices[th_idx * attend_size + num_samples] = sv_idx;
                            num_samples++;
...
void subm_strided_attention_with_hash_reuse_densemap_kernel_launcher(int x_max, int y_max, int z_max, int accu_x_stride, int accu_y_stride, int accu_z_stride, int num_voxels, int attend_size, int num_range, int hash_size,
    ...
    subm_strided_attention_with_hash_reuse_densemap_kernel<<<blocks, threads>>>(x_max, y_max, z_max, accu_x_stride, accu_y_stride, accu_z_stride, num_voxels, attend_size, num_range, hash_size,
                                                                    attend_indices, v_indices, xyz_to_vidx, range_spec);
    // cudaDeviceSynchronize();  // for using printf in kernel function
    err = cudaGetLastError();
    if (cudaSuccess != err) {
...
__global__ void subm_strided_attention_with_hash_reuse_densemap_stridetag_kernel(int x_max, int y_max, int z_max, int accu_x_stride, int accu_y_stride, int accu_z_stride, int num_voxels, int attend_size, int num_range, int hash_size,
                                                            int *attend_indices, const int *v_indices, const int *xyz_to_vidx, const int *range_spec) {
    /*
        attend_indices: [num_voxels, attend_size] for gather attend indices
        v_indices: [num_voxels, 4] bs + zyx indices of voxels
            ...
            for (int sz_idx = z_idx - z_offset; sz_idx <= z_idx + z_offset; sz_idx += z_step){
                ...
                for (int sy_idx = y_idx - y_offset; sy_idx <= y_idx + y_offset; sy_idx += y_step){
                    ...
                    for (int sx_idx = x_idx - x_offset; sx_idx <= x_idx + x_offset; sx_idx += x_step){
                        if (sx_idx >= x_max || sx_idx < 0) continue;
                        int skey = sx_idx * y_max * z_max + sy_idx * z_max + sz_idx;
                        int sv_idx = hash_table_find_reuse_densemap_stridetag(skey, accu_x_stride, hash_size, xyz_to_vidx);
                        if (sv_idx != EMPTY_KEY) { // found non-empty index
                            if (num_samples >= attend_size) return; // full and return
                            attend_indices[th_idx * attend_size + num_samples] = sv_idx;
                            num_samples++;
...
void subm_strided_attention_with_hash_reuse_densemap_stridetag_kernel_launcher(int x_max, int y_max, int z_max, int accu_x_stride, int accu_y_stride, int accu_z_stride, int num_voxels, int attend_size, int num_range, int hash_size,
    ...
    subm_strided_attention_with_hash_reuse_densemap_stridetag_kernel<<<blocks, threads>>>(x_max, y_max, z_max, accu_x_stride, accu_y_stride, accu_z_stride, num_voxels, attend_size, num_range, hash_size,
                                                                    attend_indices, v_indices, xyz_to_vidx, range_spec);
    // cudaDeviceSynchronize();  // for using printf in kernel function
    err = cudaGetLastError();
    if (cudaSuccess != err) {
