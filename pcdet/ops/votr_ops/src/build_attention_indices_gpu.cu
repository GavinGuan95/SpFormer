...
__device__ int hash_table_find_reuse_densemap_stridetag(int &key_insert, int &cur_stride, int &hash_size, const int *xyz_to_vidx) {
    int hash_idx = simple_hash(key_insert, hash_size);
    int v_idx = EMPTY_KEY;
    int prob_cnt = 0;
    while (true) {
        int prev_stride = (xyz_to_vidx[hash_idx*2 + 0] & 0x70000000) >> 28; //
        // found
...
__global__ void subm_strided_attention_with_hash_reuse_densemap_stridetag_kernel(int x_max, int y_max, int z_max, int accu_x_stride, int num_voxels, int attend_size, int num_range, int hash_size,
                                                            int *attend_indices, const int *v_indices, const int *xyz_to_vidx, const int *range_spec) {
    /*
        attend_indices: [num_voxels, attend_size] for gather attend indices
        v_indices: [num_voxels, 4] bs + zyx indices of voxels
            ...
            for (int sz_idx = z_idx - z_offset; sz_idx <= z_idx + z_offset; sz_idx += z_step){
                ...
                for (int sy_idx = y_idx - y_offset; sy_idx <= y_idx + y_offset; sy_idx += y_step){
                    ...
                    for (int sx_idx = x_idx - x_offset; sx_idx <= x_idx + x_offset; sx_idx += x_step){
                        ...
                        int key_insert = skey | (cur_stride << 28); //
                        //int sv_idx = hash_table_find(skey, hash_size, xyz_to_vidx);
                        int sv_idx = hash_table_find_reuse_densemap_stridetag(key_insert, cur_stride, hash_size, xyz_to_vidx);
                        if (sv_idx != EMPTY_KEY) { // found non-empty index
                            if (num_samples >= attend_size) return; // full and return
                            attend_indices[th_idx * attend_size + num_samples] = sv_idx;
                            num_samples++;
...
void subm_strided_attention_with_hash_reuse_densemap_stridetag_kernel_launcher(int x_max, int y_max, int z_max, int accu_x_stride, int num_voxels, int attend_size, int num_range, int hash_size,
                                                            int *attend_indices, const int *v_indices, const int *xyz_to_vidx, const int *range_spec){
    ...
    subm_strided_attention_with_hash_reuse_densemap_stridetag_kernel<<<blocks, threads>>>(x_max, y_max, z_max, accu_x_stride, num_voxels, attend_size, num_range, hash_size,
                                                                    attend_indices, v_indices, xyz_to_vidx, range_spec);
    // cudaDeviceSynchronize();  // for using printf in kernel function
    err = cudaGetLastError();
    if (cudaSuccess != err) {
