...
def _in_projection_packed(
    ...
    optimize_grouping_operation: bool = False,
) -> List[Tensor]:
    r"""
    ...
    E = q.size(-1)
    if k is v:
        assert not query_pre_calculated and not key_pre_calculated and not value_pre_calculated
        if q is k:
            # self-attention
            ...
            if key_value_group_into is None:
                ...
                if optimize_grouping_operation:
        ...
        return q if query_pre_calculated else linear(q, w_q, b_q), \
               k if key_pre_calculated else linear(k, w_k, b_k), \
               v if value_pre_calculated else linear(v, w_v, b_v)
...
def multi_head_attention_forward2(
    query: Tensor,
    key: Tensor,
    value: Tensor,
    embed_dim_to_check: int,
    ...
    if not use_separate_proj_weight:
        q, k, v = _in_projection_packed(query, key, value, in_proj_weight, in_proj_bias,
                                        query_pre_calculated, key_pre_calculated, value_pre_calculated,
                                        key_value_group_into, optimize_grouping_operation)
    else:
        assert q_proj_weight is not None, "use_separate_proj_weight is True but q_proj_weight is None"
        assert k_proj_weight is not None, "use_separate_proj_weight is True but k_proj_weight is None"
        assert v_proj_weight is not None, "use_separate_proj_weight is True but v_proj_weight is None"
